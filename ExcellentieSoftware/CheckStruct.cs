...
namespace ExcellentieSoftware
{
    ...
    public class CheckStruct
    {
        ...
        public void Run()
        {
            ...
            cLabelTela.Update();
            comm = new NpgsqlCommand("SELECT count(1) as qtde from pg_tables where tablename ='" + cTableName + "'", ds.DsCps);
        ...
        private void AnalisarContraint()
        {
            ...
            foreach (var element in alistacontraint)
            {
                ...
                if (element.cTabelaVinculo is string)
                {
                    ...
                    " WHERE " +
                    " constraint_type = " +
                    "'FOREIGN KEY' AND tc.table_name='" + cTableName.ToLower().Trim() + "'" +
                    ...
                    if (count == 0)
                    {
                        ...
                        aSql.Add("ALTER TABLE " + cTableName.ToLower().Trim() + " ADD CONSTRAINT " + cnomeindice + " " + cforekey);
                    }
                    rd.Close();
                    cmdsql.Dispose();
                }
                else if (element.cTabelaVinculo is Array)
                {
                    ...
                    csql = "SELECT count(1) as qtde " +
                        ...
                        " JOIN information_schema.constraint_column_usage AS ccu ON ccu.constraint_name = tc.constraint_name  WHERE  tc.table_name='" + cTableName.ToLower().Trim() + "' " +
                        " and tc.constraint_name = '" + cnomeindice + "'";
                    ...
                    if (count != 0)
                    {
                        aSql.Add("alter table " + cTableName.ToLower().Trim() + " drop constraint " + cnomeindice);
                    }
                    cforekey = "check (";
                    ...
                    cforekey += ")";
                    cforekey = "ALTER TABLE " + cTableName.ToLower().Trim() + " ADD CONSTRAINT " + cnomeindice + " " + cforekey;
        ...
        private void AnalisarIndices()
        {
            ...
            foreach (var element in alistaindice)
            {
                ...
                */
                ...
                if (count == 0)
                {
                    aSql.Add("create  " + IIf(element.bUnique, " unique ", " ") + " index " + cnomeindice + " on " + cTableName.ToLower().Trim() + " (" + element.cCampos + ")");
        ...
        private void AlterarArquivo()
        {
            ...
            foreach (var element in alistacampo)
            {
                ...
                if (cTableName == "cad_configuracao")
                {
                    //MessageBox.Show("oi");
                }
                if (!element.cType.ToLower().Equals("serial")) // && !element.cType.ToLower().Trim().Equals("integer"))
                {
                    ...
                    if (element.cName.ToString().ToLower().Equals(cNomeCampo))
                    {
                        ...
                        btemcampo = true;
                        csqlalteracao = "alter table " + cTableName + " alter column " + element.cName + " type ";

                    }
                    else
                    {
                        ctipodb = element.cType;
                        csqlalteracao = "alter table " + cTableName + " add column " + element.cName + " ";
                    }
                    ...
                    if (!csqlalteracao.Equals(""))
                    {
                        ...
                        {
                            if (btemcampo)
                                csqlalteracao += " ; alter table " + cTableName + " alter column " + element.cName + " set default " + element.cDefValue + ";";
                            else
                        ...
                        if (!btemcampo)
                        {
                            if (!element.cComment.Equals(""))
                            {
                                ...
                                aLegenda.Add("COMMENT ON COLUMN " + cTableName + '.' + element.cName + " is '" + element.cComment.ToString() + "'");
                            }
                        }
                    }
                }
        ...
        private void CriarArquivo()
        {
            ...
            if (!cComentario.Equals(" "))
                aSql.Add("COMMENT ON TABLE " + cTableName + " is '" + cComentario + "'");
            if (aLegenda.Count > 0)
            {
