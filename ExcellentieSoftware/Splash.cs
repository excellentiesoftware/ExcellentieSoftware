...
namespace ExcellentieSoftware
{
    public partial class SplashForm : Form
    {
        ...
        private void EstruturaCadastro(CheckStruct oCheck)
        {
            ...
            oCheck.cTableName = "cad_contrato";
            oCheck.cComentario = "Cadastr dos Contratos";

            oCheck.AddCampo("ctr_id", "serial", 0, 0, "Id Primario", false, "");
            oCheck.AddCampo("cad_id", "integer", 0, 0, "Id Cadastro", false, "");
            oCheck.AddCampo("prd_id", "integer", 0, 0, "Id Produto", false, "");
            oCheck.AddCampo("ctr_data_contrato", "date", 0, 0, "Data Contrato", false, "");
            oCheck.AddCampo("ctr_data_cancelado", "timestamp", 0, 0, "Data Cancelamento", false, "");
            oCheck.AddCampo("ctr_siuacao", "varchar", 30, 0, "Situação: Ativo/Cancelado", false, "'Ativo'");
            
            oCheck.addindice("ind_ctr_01", "cad_id,prd_id", true);

            oCheck.Run();


            oCheck.cTableName = "fin_titulo";
            oCheck.cComentario = "Titulos a Receber";

            oCheck.AddCampo("tit_id", "serial", 0, 0, "Id Primario", false, "");
            oCheck.AddCampo("cad_id", "integer", 0, 0, "Id Cadastro", false, "");
            oCheck.AddCampo("ctr_id", "integer", 0, 0, "Id Produto", false, "");
            oCheck.AddCampo("tit_data_vencimento", "date", 0, 0, "Data Vencimento", false, "");
            oCheck.AddCampo("tit_valor", "numeric", 12, 2, "Valor", false, "0");
            oCheck.AddCampo("tit_historico", "text", 0, 0, "Historico", false, "");
        ...
        private void AtualizaFuncoes(CheckStruct oCheck)
        {
            ...
            cSql = @"CREATE OR REPLACE FUNCTION exc_validacao_contrato(cnpjcpf varchar,id_produto integer)
                     RETURNS text AS
                  $BODY$
                  declare
                    cSql text = '' ;
                  ...
                  Begin
                    ...
                    cSql := 'select cad.* from cad_contrato ctr left join cad_cadastro cad on(ctr.cad_id = cad.cad_id) where cad.cad_cnpj_cpf = cnpjcpf and ctr.prd_id = id_produto' ;

                    For QrContrato in execute cSql do
